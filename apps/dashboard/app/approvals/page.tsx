...
export default function ApprovalsPage() {
  ...
  const handleDecision = async (id: string, decision: 'approved' | 'declined', customReason?: string) => {
    ...
    try {
      ...
      showNotification(
        decision === 'approved' 
          ? `Transaction approved! ${result.paymentLinkUrl ? 'Razorpay payment link generated.' : ''}`
        ...
        prev.map((item) =>
          item.id === id || item.transactionId === id
            ? {
                ...
                transaction: {
                  ...item.transaction,
                  state: decision === 'approved' ? 'approved' : 'failed',
                },
              }
            : item
        )
    ...
    <div className="space-y-8 animate-fadeIn">
        ...
        <div
          className={`fixed bottom-6 right-6 z-50 px-4 py-3 rounded-xl shadow-2xl flex items-center space-x-3 text-sm font-medium border backdrop-blur-md transition-all ${
            notification.type === 'success'
              ...
              : 'bg-blue-950/90 text-blue-300 border-blue-500/40 shadow-blue-900/30'
          }`}
        >
          {notification.type === 'success' && <CheckCheck className="w-5 h-5 text-emerald-400" />}
      ...
      {error && (
        <div className="p-4 rounded-xl bg-amber-950/30 border border-amber-500/30 flex items-start space-x-3 text-xs text-amber-300">
          ...
          <button onClick={() => fetchApprovals(true)} className="underline hover:text-amber-200">
            Retry
          </button>
        </div>
      )}
                ...
                <div className="p-5 sm:p-6 space-y-4">
                    ...
                    <div className="md:col-span-8 border-t md:border-t-0 md:border-l border-zinc-800/80 pt-3 md:pt-0 md:pl-4">
                      ...
                      <div className="flex flex-wrap items-center gap-3 mt-3">
                        <button
                          ...
                          className="flex items-center space-x-1.5 text-[11px] text-zinc-400 hover:text-zinc-200 bg-zinc-900 px-2 py-1 rounded-md border border-zinc-800 font-mono transition-colors"
                        >
                  ...
                  <div>
                    <button
                      ...
                      className="flex items-center justify-between w-full py-1.5 text-xs text-zinc-400 hover:text-zinc-200 font-medium transition-colors"
                    >
                  ...
                  {!isDecided ? (
                    ...
                    <div className="pt-2 border-t border-zinc-800/60 flex items-center justify-between text-xs text-zinc-400">
                      <span>Decided by {approval.decidedBy || 'human:admin'}</span>
                      <span className="font-mono text-[11px] text-zinc-400">
                        Tx State: <span className="text-zinc-300 font-semibold">{tx?.state}</span>
                      </span>
                    </div>
                  )}
                </div>
...
function getFallbackApprovals(): PendingApproval[] {
  return [
    ...
    },
    {
      ...
      transaction: {
        id: 'txn_demo_9822',
        correlationId: 'cid_demo_bakery_002',
        mandateId: 'mnd_grocery_02',
        quoteId: 'quot_bakery_11',
        state: 'gated',
        createdAt: new Date(Date.now() - 15 * 60 * 1000).toISOString(),
        quote: {
          id: 'quot_bakery_11',
          items: [
            { sku: 'ghee-500ml', qty: 2, unitPrice: 45000, name: 'Pure ghee, 500ml' },
          ],
          total: 90000, // ₹900.00
          expiresAt: new Date(Date.now() + 12 * 60 * 1000).toISOString(),
          createdAt: new Date(Date.now() - 15 * 60 * 1000).toISOString(),
        ...
        mandate: {
          id: 'mnd_grocery_02',
          agentId: 'agt_kitchen_bot',
          merchantId: 'mch_new_bakery',
          maxPerTransaction: 200000,
          dailyCap: 200000,
          autoApproveThreshold: 50000,
          allowedCategories: ['grocery.dairy'],
          agent: {
            id: 'agt_kitchen_bot',
            name: 'Pantry Auto-Replenisher Bot',
          },
        },
        auditLogs: [
          {
            ...
            step: 'gate_decision',
            decision: 'pending',
            reason: 'first transaction with this merchant requires human approval',
            ruleId: 'gate_first_time',
            actor: 'system',
            createdAt: new Date(Date.now() - 15 * 60 * 1000).toISOString(),
          },
        ],
